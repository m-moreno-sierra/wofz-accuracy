...
int main(int argc, char* argv[])
{
    if (argc != 5) {
        ...
        fprintf(stderr,
                "  M_offsets = M : try neighbor grid points from -M to M as expansion centers\n");
    ...
    const std::vector<std::vector<int>> S{::sorted_diameters(0, 0, kappamax),
    ...
    std::vector<std::pair<int, int>> I =
        Ref::domain_grid(inv_b); // List of grid points within domain.
